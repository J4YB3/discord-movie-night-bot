...
use crate::movie_behaviour::{
    SortedMovieList, UserSortedMovieListVectorEntry, WatchListEntry, get_movie_link,
};
...
pub fn show_watch_list(bot_data: &mut crate::BotData, order: String) {
    let message = bot_data
        .message
        .as_ref()
        .expect("Passing message to show_watch_list function failed.");
    ...
    if let Some((idx, message)) =
        bot_data
            ...
            .enumerate()
            .find_map(|(idx, x)| {
                if let crate::general_behaviour::WaitingForReaction::WatchListPagination(
                    message,
                    _,
                    _,
                ) = x
    ...
    if watch_list_count > 0 {
        ...
        if order == "user" {
            ...
            total_pages = user_sorted_watch_list
                .iter()
                .map(|x| x.number_of_pages_required)
                .sum();

            watch_list_string +=
                generate_user_sorted_watch_list_page_string(&user_sorted_watch_list, 1).as_str();

            sorted_watch_list_enum_option = Some(SortedMovieList::WatchListUserSorted(
                total_pages,
        ...
        // If the ordering should be by id
            ...
            let id_sorted_watch_list: Vec<(u32, WatchListEntry)> = bot_data
                .watch_list
                .iter()
                .sorted()
                .filter_map(|(id, entry)| {
                ...
                .collect();
            total_pages = (id_sorted_watch_list.len() as f64 / crate::MAX_ENTRIES_PER_PAGE as f64)
                .ceil() as usize;

            watch_list_string +=
                generate_id_sorted_watch_list_page_string(&id_sorted_watch_list, 1).as_str();

            sorted_watch_list_enum_option = Some(SortedMovieList::WatchListIdSorted(
                total_pages,
...
pub fn generate_user_sorted_watch_list_page_string(
    ...
    if watch_list_count == 1 {
        watch_list_string += format!(
            "Es ist zur Zeit **{}** Film auf der Liste\n\n",
            watch_list_count
        )
        .as_str();
    } else {
        watch_list_string += format!(
            "Es sind zur Zeit **{}** Filme auf der Liste\n\n",
            watch_list_count
        )
        .as_str();
    }
 ...
 */
fn create_user_sorted_watch_list(
 ...
 */
fn create_user_sorted_watch_list_vector(
    bot_data: &crate::BotData,
    ...
    user_sorted_watch_list_hash_map
        .iter()
 ...
 */
pub fn generate_id_sorted_watch_list_page_string(
    id_sorted_watch_list: &Vec<(u32, WatchListEntry)>,
    page_to_show: usize,
) -> String {
    let mut watch_list_string = String::new();
