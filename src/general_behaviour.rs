    ...
    if is_user_administrator(bot_data, message.author.id) {
        ...
        let _ =
            bot_data.bot.send_embed(message.channel_id, "", |embed| {
                embed.title(":information_source: Rechte nicht ausreichend").description(
                format!(
...
pub fn is_user_administrator(bot_data: &crate::BotData, user_id: discord::model::UserId) -> bool {
    let author_role_ids = bot_data
        .bot
        .get_member(bot_data.server_id, user_id)
        .expect("Retrieval of author user failed.")
        .roles;
